...
(function($) {
	...
	$.subscribe('viewportResizeEnd_prioritize', function () {
...
(function($) {
	$.extend(bb,{
		billingIndicator: {
			...
			$billingIndicatorPlaceholder: null,
			billingIndicatorOffset: 0,
			billingIndicatorFixedClass: 'billing-indicator-fixed',
			billingIndicatorPlaceholderClass: 'billing-indicator-placeholder',
			inClass : 'billing-indicator-in',
			setGlobal: function (bb) {
				var self = this;
				self.bb = bb;
			},
			init : function () {
				var self = this;
				self.$billingIndicator = $('.billing-indicator');
				if(!self.$billingIndicator.length) {
					self.$billingIndicator = null;
					return;
				}
				// create the placeholder and add it before the nav
				self.$billingIndicatorPlaceholder = $('<div />', {
					'class': self.billingIndicatorPlaceholderClass
				});
				self.$billingIndicator.before(self.$billingIndicatorPlaceholder);
				// monitor the scroll
				self.bb.settings.$main.on('scroll.pageHeader', function () {
					if(self.bb.monitorMq.currentBreakpoint >= self.bb.settings.breakPointD) {
						self.checkPosition();
					}
				});
			},
			checkPosition: function () {
				var self = this;
				if(!self.$billingIndicator || !self.$billingIndicator.is(':visible')) {
					return;
				}
				var pageOffset = self.bb.settings.$main.offset().top,
					pageScrollPosition = self.bb.settings.$main.scrollTop(),
					billingIndicatorFromTop = Math.round(self.$billingIndicatorPlaceholder.offset().top - pageOffset);
				if (billingIndicatorFromTop <= 0) {
					self.$billingIndicator.css('top', pageScrollPosition);
					self.bb.settings.$html.addClass(self.billingIndicatorFixedClass);
					self.$billingIndicatorPlaceholder.height(self.$billingIndicator.outerHeight());
				} else {
					self.bb.settings.$html.removeClass(self.billingIndicatorFixedClass);
					self.$billingIndicator.removeAttr('style');
					self.$billingIndicatorPlaceholder.height(0);
				}
			}
		}
	});
	$.subscribe('setGlobal', function (event, bb) {
		bb.billingIndicator.setGlobal(bb);
	});
	$.subscribe('pageReady', function () {
		bb.billingIndicator.init();
	});
}(jQuery));
	...
	$.extend(bb,{
		masonryLayout : {
			...
			preInit: function() {
				...
				if (self.$masonryContainer.length > 0) {
					self.init();
				}
			},
			init: function() {
				var self = this,
					sizerWidthClass = null;
				self.$masonryBlocks = self.$masonryContainer.find('.block');
				if(self.$masonryContainer.data('sizer')){
					sizerWidthClass = self.$masonryContainer.data('sizer');
				}
				self.$masonrySizer = $('<div class="block-sizer ' + sizerWidthClass +'" /><div class="gutter-sizer" />');
				self.$masonryContainer.append(self.$masonrySizer);
				self.buildMasonry();
				console.log(bb.monitorMq.currentBreakpoint + ' >= ' + bb.settings.breakPointD);
				if(self.bb.monitorMq.currentBreakpoint >= self.bb.settings.breakPointD) {
					for (var i = 0; i < self.$masonryBlocks.length; i++) {
						var delay = parseInt(i * 200, 10);
						//console.log(delay);
						self.showBlock(self.$masonryBlocks.eq(i),delay);
					}
				}
			},
			...
			buildMasonry: function() {
				var self = this;
				if (self.$masonryContainer.length < 1) {
					return false;
				}
				self.$masonryContainer.addClass('loading');
				self.$masonryContainer.imagesLoaded(function() {
					self.$masonryContainer.removeClass('loading');
					self.$masonryContainer.packery({
						columnWidth: '.block-sizer',
						gutter: '.gutter-sizer',
						itemSelector: '.block'
					});
					self.masonry = self.$masonryContainer.data('packery');
				});
			}
		}
	});
	$.subscribe('setGlobal', function (event, bb) {
		bb.masonryLayout.setGlobal(bb);
	});
	$.subscribe('pageReady', function () {
		bb.masonryLayout.preInit();
	});
	$.subscribe('viewportResizeEnd', function () {
		//bb.masonryLayout.buildMasonry();
	});
}(jQuery));
	...
	$.extend(bb,{
		search : {
			...
			init : function () {
				...
				self.$handleClose = $('.action-search-close'),
				self.$handleClose.on('click.search', function (event) {
					event.preventDefault();
					if (self.bb.settings.$html.hasClass(self.searchInClass)) {
						self.closeSearch(event);
					}
			...
			openSearch : function (event) {
				var self = this;
				self.bb.settings.$html.addClass(self.searchShowClass);

				self.showTimeout = setTimeout(function () {
					self.bb.settings.$html.addClass(self.searchInClass);
			...
			closeSearch : function (event) {
				...
				self.hideTimeout = setTimeout(function () {
					self.bb.settings.$html.removeClass(self.searchShowClass);
					clearTimeout(self.hideTimeout);
				}, 300);
			}
		}
		...
		toggle: {
			...
			init: function () {
				...
				self.$toggles.on('click.toggle', '.toggle-handle', function (event) {
					...
					$toggle.toggleClass(self.toggleActiveClass);
				});
			}
		}
	});
